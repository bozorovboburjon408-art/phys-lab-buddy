...
const Laboratories = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      const cleanedData = {
        ...
        purpose: formData.purpose_uz,
        purpose_uz: formData.purpose_uz,
        equipment: formData.equipment_uz.filter(e => e.trim() !== ""),
        equipment_uz: formData.equipment_uz.filter(e => e.trim() !== ""),
        theory: formData.theory_uz,
        theory_uz: formData.theory_uz,
        procedure: formData.procedure_uz.filter(p => p.trim() !== ""),
  ...
  const addArrayItem = (field: "equipment_uz" | "procedure_uz") => {
    setFormData(prev => ({ ...prev, [field]: [...prev[field], ""] }));
  };

  const removeArrayItem = (field: "equipment_uz" | "procedure_uz", index: number) => {
    setFormData(prev => ({ ...prev, [field]: prev[field].filter((_, i) => i !== index) }));
  };

  const updateArrayItem = (field: "equipment_uz" | "procedure_uz", index: number, value: string) => {
                  ...
                  <div className="space-y-2">
                    {formData.table_columns.map((col, i) => (
                      <div key={i} className="flex gap-2 items-center p-2 rounded-lg bg-secondary/30">
                        ...
                        <Input value={col.nameUz} onChange={e => {
                          updateTableColumn(i, "nameUz", e.target.value);
                          updateTableColumn(i, "name", e.target.value);
                        }} placeholder="Nomi" className="flex-1" />
